...
def authenticate(username, password):
    ...
    """
    user = UserModel.find_by_username(username)  # ищем юзера по имени
    if user and safe_str_cmp(user.password, password):  # сравниваем пароли
        return user


def identity(payload):  # when user is already logged in
    """

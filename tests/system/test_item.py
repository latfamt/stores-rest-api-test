...
class ItemTest(BaseTest):
    def setUp(self):
        super(ItemTest, self).setUp() # запустить setUpClass из BaseTest
        with self.app() as client:
            with self.app_context():
                UserModel('test', '1234').save_to_db()  # создаем юзера
                auth_request = client.post('/auth',
                                           data=json.dumps({'username': 'test', 'password': '1234'}),
                                           headers={'Content-Type': 'application/json'})
                auth_token = json.loads(auth_request.data)['access_token'] # our jwt - JSON Web Token
    ...
    def test_delete_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                ItemModel('test', 19.99, 1).save_to_db()
                resp = client.delete('/item/test') # авторизация нужна только для get
                self.assertEqual(resp.status_code, 200)
    ...
    def test_create_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                resp = client.post('/item/test', data={'price': 17.99, 'store_id': 1})  # авториз. нужна только для get
    ...
    def test_put_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                resp = client.put('/item/test', data={'price': 17.99, 'store_id': 1})  # авториз. нужна только для get
                self.assertEqual(resp.status_code, 200)
    ...
    def test_item_list(self):
        with self.app() as client:
            with self.app_context():
                ...
                StoreModel('test2store').save_to_db()
                ItemModel('test', 19.99, 1).save_to_db()
                ItemModel('test2', 29.99, 2).save_to_db()
